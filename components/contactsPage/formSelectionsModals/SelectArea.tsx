...
type BottomModalSheetProps = {
  ...
  sAreaField: sAreaType[];
  selectedArea: sAreaType | null;
  setSelectedArea: (value: sAreaType | null) => void;
  // setsAreaField: (value: sAreaType[]) => void;
}

const SelectArea = (({ ref, sAreaField, selectedArea, setSelectedArea }: BottomModalSheetProps) => {

  const [searchQuery, setSearchQuery] = useState('');

  const filteredAreaField = sAreaField.filter((item) =>
  ...
  return (
    ...
    >
      <View className="px-6 mb-[12px]">
        <Text onPress={() => ref?.current?.close()} className="text-lg text-blue-400">Close</Text>
      </View>
      <BottomSheetView className="justify-between gap-[20px] px-4 pb-20 h-full " >
        ...
        <ScrollView className="h-full">
          ...
          />
          {/* <ListOptionCheckBox className="!bg-transparent" isChecked={sAreaField} onPress={() => {
            setsAreaField(true);
            ref?.current?.close();
          }} title="Active" /> */}
