...
const ListOptionCheckBox = ({ children, title = '', titleMarginLeft, titleClassName, isChecked, ...props }: ListOptionCheckBoxProps) => {
  ...
  return (
    <Pressable
      ...
      {isChecked ? (
        <MaterialCommunityIcons name="checkbox-marked-circle-outline" size={21} color={colorScheme == 'dark' ? '#50a2ff' : '#2563eb'} />
      ) : (
        <MaterialCommunityIcons name="checkbox-blank-circle-outline" size={21} color={colorScheme == 'dark' ? '#f8f8f8' : '#818792'} />
      )}
    </Pressable>
  );
};
