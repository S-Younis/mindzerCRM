    ...
    return (
        <Pressable  {...props} className={`bg-[#161f2e] border-[#262f3a]  h-[66px]  px-4 border-b-[1px] flex-row items-center justify-between gap-4 ${props.className} active:opacity-70`}>
            <View className='flex-row items-center gap-[14px] w-[100%]  '>
                ...
                <Pressable onPress={() => !isReadOnly && textInputRef.current?.focus()} className=' gap-[2px] w-[100%] pr-2 '>
                    <Text className="  text-xs text-gray-400  ">{title}</Text>
                    <TextInput style={{paddingHorizontal:0 }} ref={textInputRef} onChangeText={onChangeText} onBlur={onBlur} clearButtonMode='while-editing' readOnly={isReadOnly} value={value} defaultValue={value} className="text-dark    pr-[10px] py-0  dark:text-light h-[22px] " />
                </Pressable>
            </View>
