...
const BottomModalSheet = ({ ref }: BottomModalSheetProps) => {
  ...
  const showCreateOpp = () => {
    router.push('(modals)/opps/createOppPage');
    ref?.current?.close();
  };
  ...
  return (
    <BottomSheet
      ...
      <BottomSheetView className="flex gap-4 p-4   ">
        <MindzerButton isTitleCentered variants="primary" className="w-full" onPress={showCreateOpp}>
          <MaterialCommunityIcons name="handshake-outline" size={18} color="white" className="mr-2" />
          {/* <AntDesign name="adduser" size={18} color="white" className="mr-2" /> */}
          <Text className={`font-medium text-white `}>Create Opportunity</Text>
        </MindzerButton>
