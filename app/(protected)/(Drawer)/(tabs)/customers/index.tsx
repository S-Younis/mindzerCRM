        ...
        {lst_customers.length > 0 && (
          <FlashList
            ...
            renderItem={({ item, index }) => (
              <CustomerCard
                onPress={() => {
                  router.push(`/customers/${item.iCustomerId}`);
                }}
                className={`${index == 0 ? 'mt-4' : index == lst_customers.length - 1 ? 'mb-4' : ''}`}
                sCustomer={item.sCustomer}
                sContactCount={item.sContactCount}
                sOppsCount={item.sOppsCount}
