...
const importContactPage = () => {
    ...
    const handleSelection = () => {
        ...
        router.push('/contacts/createContact?importContact=true');
    }
    ...
    return (
        <>
            ...
            <Stack.Screen options={{
                ...
                headerLeft: () => {
                    if (Platform.OS == 'ios') { return <Text className=' text-xl text-blue-400' onPress={() => router.back()} >Cancel</Text> } else return null;
                },
                headerRight: () => {
                    if (checkedContacts.length == 1) return <Text className='text-blue-400 text-xl' onPress={handleSelection}  >Select</Text>
                    else null;
                }
            }
            } />
                ...
                <View className='flex-1' style={{ paddingBottom: 10 }} >
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            <ListOptionCheckBox key={item.id} isChecked={checkedContacts.includes(item.id)} onPress={() => handleCheckBox(item.id)} title={` ${item.name || ''}`} className={`${index == contacts.length - 1 && 'rounded-br-lg rounded-bl-lg'}`}  >
